    ...
    if (
        ...
        def quantity(event):
            ...
            def total(stuff1_sno, stuff1_name, stuff1_rate, stuff1_qty, stuff1_price):
                ...
                times = 0
                DOUBLE = 0
                list_items.grid_forget()
                # place_forget

                list_items = tk.Canvas(
                    main, width=0, height=0, bg="white", highlightthickness=0
                )
                list_items.place(x=10, y=250)
                ...
                def redefining(times):
                    ...
                    if times < len(l):
                        ...
                        n_1 = len(l)
                        cursor.execute(
                            "select * from items where s_no=" + str(l[times][0])
                        )
                        data_new = cursor.fetchall()
                        stuff1_sno = tk.Entry(
                            list_items, width=16, font=("calibre", 15)
                        )
                        stuff1_sno.insert(0, data_new[0][0])
                        stuff1_sno.config(state="disabled")
                        stuff1_sno.grid(row=DOUBLE, cloumn=0)
                        stuff1_name = tk.Entry(
                            list_items, width=57, font=("calibre", 15)
                        )
                        stuff1_name.insert(0, data_new[0][1])
                        stuff1_name.config(state="disabled")
                        stuff1_name.grid(row=DOUBLE, column=1)
                        stuff1_rate = tk.Entry(
                            list_items, width=25, font=("calibre", 15)
                        )
                        stuff1_rate.insert(0, data_new[0][2])
                        stuff1_rate.config(state="disabled")
                        stuff1_rate.grid(x=DOUBLE, column=2)
                        ...
                        stuff1_qty = tk.Entry(
                            list_items, width=10, font=("calibre", 15)
                        )
                        ...
                        stuff1_price = tk.Entry(
                            list_items, width=12, font=("calibre", 15)
                        )
                        stuff1_price.insert(
                            0, str(int(data_new[0][2]) * float(l[times][-1]))
                        )
            ...
            if stuff1_qty.get() == "":
                ...
                message = tk.Label(
                    popup1,
                    text="Please check the quantity entered",
                ...
                ok1 = tk.Button(
                    popup1,
                    text="Ok",
                    command=lambda: [popup1.destroy(), stuff1_qty.focus()],
                )
                ok1.pack()
                ok1.focus()
            else:
                ...
                """dlt=tk.Button(main_frame,state='normal',width=2,height=1,command=twofns,highlightthickness=0)
                       dlt.grid(row=0,column=5)"""
...
def member():
    ...
    def checking(data_member):
        ...
        if (
            ...
            ok2.destroy()
            ok3 = tk.Button(
                popup2, text="OK", font=("calibre", 15, "bold"), command=discount_total
            )
    ...
    use_card = tk.Button(
        popup2, text="USE CARD", font=("verdana", 20), command=lambda: card_use(ttt)
    ).grid(row=1, column=3, stick="e")
    heading_bill = tk.Label(popup2, text="BILL", font=("verdana", 30)).grid(
