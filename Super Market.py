...
def rows(event):
       ...
       if (s_no.get() > str(25000) and s_no.get() < str(25101) and int(s_no.get()) not in l_nolist) or (name.get()!=""):
               ...
               main_frame=tk.Frame(list_items)
               ...
               if len(l)<=3:
                   ...
                   bar=tk.Scrollbar(list_items, command=list_items.yview) 
                   bar.place(x=200,y=200,relheight=1,anchor='ne')
               ...
               stuff1_qty.bind('<Return>',quantity)
               stuff1_price=tk.Entry(main_frame,width=12,font=('calibre',15),state='disabled')
               stuff1_price.config(state='disabled')
