...
#frame=tk.Frame(list_items)
#list_items=tk.Canvas(frame,width=0,height=0,bg='white',highlightthickness=0)
...
frame=tk.Frame(list_items)
bar=ttk.Scrollbar(main,orient='vertical')
bar.pack(side='right',fill='y')
bar.config(command=list_items.yview)
list_items.config(yscrollcommand=bar)
...
def bill():
    ...
    billllll.title("BILL")
    '''prdt=tk.Label(bill_final,text="PRODUCTS",font=('cmabria',10)).grid(row=0,column=0)
    prdt=tk.Label(bill_final,text="RATE",font=('cmabria',10)).grid(row=0,column=1)
    prdt=tk.Label(bill_final,text="QUANTITY",font=('cmabria',10)).grid(row=0,column=2)
    prdt=tk.Label(bill_final,text="PRICE",font=('cmabria',10)).grid(row=0,column=3)
    r=1
    c=0'''
    ...
    prdt=tk.Label(billllll,text=bill_final,font=('courier new',10)).grid(row=0,column=0)
    '''print(bill_items)
        prdt=tk.Label(bill_final,text=str(bill_items[1]),font=('cmabria',10)).grid(row=r,column=c)
        prdt=tk.Label(bill_final,text=str(bill_items[2]),font=('cmabria',10)).grid(row=r,column=c+1)
        prdt=tk.Label(bill_final,text=str(l_qty[l.index(bill_items)-1]),font=('cmabria',10)).grid(row=r,column=c+2)
        prdt=tk.Label(bill_final,text=str(float(l_qty[l.index(bill_items)-1])*float(bill_items[2])),font=('cmabria',10)).grid(row=r,column=c+3)
        r+=1'''
...
def member():
        ...
        if entry_member_name.get() in list_of_users or entry_member_no.get() in list_of_no:
            ...
            if entry_member_name.get()=="":
                for p in data_member:
                    if p[2]==entry_member_no.get():
                        user_Name=p[1]
                        entry_member_name.insert(0,p[1])
                        entry_member_name.config(state="disabled")
            else:
                for p in data_member:
                    if p[1]==entry_member_name.get():
                        user_No=p[2]
                        entry_member_no.insert(0,user_No)
                        entry_member_no.config(state="disabled")
            ttt=float(total_l.cget("text")[1:])
            print(total_l.cget("text"))
            print(data_member)
            if ttt>1000 and float(data_member[0][3])>0:
                ttt=(data_member[4]/100)*ttt-ttt
                print(ttt)
                cur.execute("update users set points=points+5 where mobile_number='" + data_member[2] + "'")
                ms.commit()
            #elif ttt>500 and ttt<=1000 and float(data_member[0][3])>0:
                
                
            ok2.destroy()
            ok3=tk.Button(popup2,text="OK",font=('calibre',15,'bold'),command=des)
            ok3.grid(row=5,column=2,sticky='e')
            ...
            popup2.lift()
            if msg=='yes':
                ok4=tk.Button(popup2,text="OK",font=('cambria',15,'bold'),command=des)
    ...
    popup2=tk.Tk()
    name=tk.Label(popup2,text="Name:",font=('verdana'))
    name.grid(row=1,column=1)
    entry_member_name=tk.Entry(popup2,width=50)
    entry_member_name.grid(row=1,column=2)
...
bc=tk.StringVar()
black_can=tk.Canvas(main,height=350,width=350,highlightthickness=0,bg='black')
black_can.place(x=1000,y=200)
t_black=tk.Label(black_can,justify='center',text="OFFERS",fg='white',bg='black',font=('cambria',20))
t_black.place(x=125,y=5)
disc="~ Pineapple great offer buy one get one free                             "*100
cx=cx1=340
cy=35
def defe():
    global cx,cy
    l_black=tk.Label(black_can,text=disc,fg='white',bg='black',font=('cambria',10))
    l_black.place(x=cx,y=cy)
    def run():
        global cx,cy,cx1
        cx-=1
        l_black.place(x=cx,y=cy)
        '''while cx<=-10:
            ...
            cx1-=1
            l_black.place(x=cx,y=cy)
            l1_black=tk.Label(black_can,text=disc,fg='white',bg='black',font=('cambria',10))
            l1_black.place(x=100,y=cy)
        ...
        if cx<=-10:
            cx1=340
            l1_black=tk.Label(black_can,text=disc,fg='white',bg='black',font=('cambria',10))
            l1_black.place(x=cx1,y=cy)
            def run1():
                global cx1
                cx1-=1
                l1_black.place(x=cx1,y=35)
                main.after(10,run)
            main.after(500,run1())'''
